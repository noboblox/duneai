...
public:
    ...
    void reachable(AreaId from, int storm, int movement, std::vector<AreaId>& result) const;
    bool insideStorm(AreaId id, int storm) const;
...
private:
	...
	bool sameTerritory(AreaId l, AreaId r) const noexcept { return (l / 10) == (r / 10); }
    void neighbors(AreaId from, int storm, const std::vector<AreaId>& exclude, std::vector<std::pair<AreaId, int>>& result) const;
    const Area* getArea(AreaId id) const;
};
