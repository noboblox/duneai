...
void Arrakis::reachable(AreaId from, int storm, int movement, std::vector<AreaId>& result) const
{
	result.clear();
	...
	while (!current.empty())
	{
		neighbors(current.front().first, storm, visited, next);

		for (const auto& dest : next)
		{
			visited.push_back(dest.first);
...
void Arrakis::neighbors(AreaId from, int storm, const std::vector<AreaId>& exclude, std::vector<std::pair<AreaId, int>>& result) const
{
	result.clear();
	...
	for (; it != connections.cend() && it->from == from; ++it)
	{
		if (insideStorm(it->to, storm))
			continue;
		if (std::find(exclude.cbegin(), exclude.cend(), it->to))
			continue;
...
bool Arrakis::insideStorm(AreaId id, int storm) const
{
	if (storm == 0)
		return false;
...
const Arrakis::Area* Arrakis::getArea(AreaId id) const
{
